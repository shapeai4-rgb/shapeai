...
export function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatus('error');
      setErrorMessage(messages.contact.error);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label htmlFor="name" className="block text-sm font-medium text-neutral-slate">{messages.contact.name}</label>
      ...
      <div>
        <label htmlFor="email" className="block text-sm font-medium text-neutral-slate">{messages.contact.email}</label>
      ...
      <div>
        <label htmlFor="message" className="block text-sm font-medium text-neutral-slate">{messages.contact.message}</label>
      ...
      {status === 'success' && (
        <p className="text-status-success">{messages.contact.success}</p>
      )}
      {status === 'error' && (
