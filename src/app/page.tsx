...
const STEP_ITEMS = [
  ...
  {
    title: "Preview",
    desc: "See calories, macros, portions and meal structure before you download.",
    icon: (
      <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="1.5">
  ...
  {
    title: "7-Day PDF",
    desc: "Download a branded PDF with a shopping list and QR to the live plan.",
    icon: (
      <svg viewBox="0 0 24 24" className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="1.5">
  ...
  return (
    <motion.div variants={itemVariants} className="rounded-card border border-neutral-lines bg-white p-5">
      <button onClick={() => setIsOpen(!isOpen)} className="flex w-full items-center justify-between text-left">
        ...
        <motion.span className="text-neutral-slate" animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.2 }}>
          v
        </motion.span>
      </button>
      <AnimatePresence>
