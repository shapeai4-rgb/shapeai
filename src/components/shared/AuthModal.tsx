...
export function AuthModal({ open, mode, onClose, onModeChange }: AuthModalProps) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (mode === 'signup') {
      try {
        ...
        if (result?.ok) {
          ...
          setErrors({ api: messages.auth.registrationLoginFailed });
        }
        ...
        if (error instanceof AxiosError && error.response) {
          ...
          setErrors({ api: messages.auth.unexpectedError });
        }
      }
    }
    ...
    if (mode === 'login') {
      ...
      if (result?.ok && !result.error) {
        ...
        setErrors({ api: messages.auth.invalidCredentials });
      }
    }
    setLoading(false);
  };
  ...
  return (
    <Modal open={open} onClose={onClose} title={mode === 'login' ? messages.auth.loginTitle : messages.auth.createTitle}>
      <form onSubmit={submit} className="grid gap-4 text-sm" noValidate>
        {mode === 'signup' && (
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="font-medium text-neutral-slate">{messages.auth.firstName}</label>
            ...
            <div>
              <label className="font-medium text-neutral-slate">{messages.auth.lastName}</label>
            ...
            <div className="col-span-2">
              <label className="font-medium text-neutral-slate">{messages.auth.dateOfBirth}</label>
        ...
        {mode === 'signup' && (
          <div>
            <label className="font-medium text-neutral-slate">{messages.auth.phone}</label>
        ...
        <div>
          <label className="font-medium text-neutral-slate">{messages.auth.password}</label>
        ...
        {mode === 'signup' && (
          <div className="grid gap-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="font-medium text-neutral-slate">{messages.auth.street}</label>
              ...
              <div>
                <label className="font-medium text-neutral-slate">{messages.auth.city}</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="font-medium text-neutral-slate">{messages.auth.country}</label>
                <select value={country} onChange={(e) => setCountry(e.target.value)} required className="mt-1 w-full rounded-lg border border-neutral-lines px-3 py-2 outline-none ring-accent/50 focus:ring-2 bg-white">
                  <option value="" disabled>{messages.auth.selectCountry}</option>
                  {ALLOWED_COUNTRIES.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div>
                <label className="font-medium text-neutral-slate">{messages.auth.postCode}</label>
        ...
        {mode === 'signup' && (
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="terms" className="text-xs text-neutral-slate"> {messages.auth.termsAgreement}</label>
          </div>
        )}

        <div className="mt-2 flex flex-col gap-3">
          <Button type="submit" disabled={loading || !canSubmit}>
            {loading ? messages.auth.processing : messages.common.continue}
          </Button>
          ...
          <button type="button" onClick={() => signIn('google', { callbackUrl: '/dashboard' })} className="flex items-center justify-center gap-2 rounded-xl border border-neutral-lines bg-white px-4 py-2 font-semibold text-neutral-ink transition-colors hover:bg-neutral-mist">
            ...
            <span>{messages.auth.continueWithGoogle}</span>
          </button>
        </div>

        <div className="mt-2 text-center text-xs text-neutral-slate">
          {mode === 'login' ? messages.auth.dontHaveAccount : messages.auth.alreadyHaveAccount}
          <button
            type="button"
            className="ml-1 font-semibold text-accent hover:underline"
            onClick={() => onModeChange?.(mode === 'login' ? 'signup' : 'login')}
          >
            {mode === 'login' ? messages.auth.signup : messages.header.login}
          </button>
        </div>
      </form>
    </Modal>
